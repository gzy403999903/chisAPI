 ...
 * @Version 1.0
 */
public interface SelfUsedRecordService {

    /**
     * 保存操作
     * @param selfUsedRecordList
     */
    @Transactional
    void saveList(List<SelfUsedRecord> selfUsedRecordList);
     ...
     * @return
     */
    default List<SelfUsedRecord> parseToObjectList(List<Map<String, Object>> listMap) {
        String json = JSONUtils.parseObjectToJson(listMap);
        return JSONUtils.parseJsonToObject(json, new TypeReference<List<SelfUsedRecord>>() {});
    }
     ...
     * @return
     */
    default Boolean examineApproveState(List<SelfUsedRecord> list, Byte approveState) {
        if (list.isEmpty()) {
            throw new RuntimeException("领用明细不能为空");
        }

        for (SelfUsedRecord selfUsedRecord : list) {
            if (selfUsedRecord.getApproveState().byteValue() != approveState.byteValue()) {
                return false;
            }
        }
        return true;
